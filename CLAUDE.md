...
- **`GET /teams/current` is gone from the spec but not from the router.** The vendored spec (re-vendored 2026-09-09 against upstream `main`, 275 operations) documents `GET /team` and `GET /team/members` instead. Upstream still routes `/teams/current` and `/teams/current/members` as deprecated aliases, and `/team` only exists from Coolify 4.3.0, so the client and HTTP mode's proof-of-access keep calling `/teams/current` while 4.0.x is in the tested range. `check:spec-drift` is method-blind and matches `/teams/current` against `/teams/{id}` by segment count, so it will not flag this; switch to `/team` only when the tested range drops below 4.3.
