...
- **Destructive operations now ask the human, not the model** (#261) — `stop_all_apps` was gated on a `confirm: true` parameter that the _model_ fills in, which is the model confirming with itself before taking every application on the estate down. On clients that support [elicitation](https://modelcontextprotocol.io/specification/2025-06-18/changelog) the confirmation now happens in client UI, outside the model's control.
  ...
  **The tool call's abort signal is threaded into the elicitation**, which matters more than it sounds. The prompt runs _inside_ the `tools/call` request and the SDK's client-side default request timeout is 60 seconds — shorter than a human takes to read "stop ALL 12 applications?" and decide. Without the signal, a client that gives up at 60s leaves the prompt live for another four minutes, and an accept at t=90s executes the destructive operation with nobody listening, after the model has already been told the call failed and may have retried it. There is a regression test that fails without the signal threaded.
  ...
  Prompts state their blast radius rather than asking in the abstract — "take down 12 running applications (api, worker, cockpit, umami, scheduler, mailer, search, billing and 4 more) across 3 servers?" — and the lookup that produces those counts is lazy, so clients that will never show the question do not pay for the extra API call. Names are truncated after eight, because the point of the list is spotting the one production app that should not be in the set, and sixty names defeats that as thoroughly as none.
  ...
  **Strictly progressive enhancement.** Client support is uneven — Claude Code and VS Code Copilot have it, Claude Desktop and claude.ai do not yet — so the server checks the client's advertised `elicitation` capability at runtime and, when it is absent, proceeds exactly as before with the existing parameter guards. A client that cannot be asked is not a client that gets blocked. Once a client _does_ advertise support it fails closed instead: a decline, a cancel, a timeout or a transport error all abort the operation. The one failure that does not abort is the blast-radius lookup itself, because a summary we could not compute is a reason to ask a vaguer question, not to skip asking.
  ...
  Prompts are calibrated to what the operation can actually do, because one that overstates its danger teaches the same habit as one raised too often. Deleting an environment says outright that Coolify refuses a non-empty one (documented `400`), so it only ever succeeds against an empty environment.

  Deleting a project goes the other way and names **applications, databases and services**, because the API documents no "project has resources" refusal, so the delete is assumed to cascade over all three. Counting only applications would have been the understating direction a confirmation must never be wrong in — verified on the test estate, where three projects hold databases or services and no applications at all, and would each have been introduced with "no applications in it" immediately before destroying live data. `Database.environment_id` and `Service.environment_id` are added to the types for this: verified live against 4.1.2, both list endpoints return the numeric environment link and leave `project_uuid` undefined, exactly as `GET /applications` does.
...
- **`npm run test:integration` could not run at all** — Jest 30 renamed `--testPathPattern` to `--testPathPatterns`, and the script still passed the singular form, so every invocation exited on an unrecognised-option error without executing a single test. That is the command `/smoke-test` and `CLAUDE.md` both name as the way to verify a fix against a live Coolify, which means the documented verification path has been dead since the Jest 30 bump. The run also tripped the 80% global coverage threshold and reported failure even when all tests passed — an integration run exercises a thin slice of the client by design — so `--coverage=false` is set alongside the rename.
...
- **`isRunningStatus` extracted from `stopAllApps`, and its substring bug fixed** — the "which apps count as running" predicate was inline in `stopAllApps` and is now shared with the confirmation prompt. Two copies would eventually disagree, and the failure mode of that is a confirmation dialog understating its own blast radius.

  The extraction also surfaced a long-standing over-match worth fixing rather than inheriting: `'unhealthy'` contains `'healthy'`, so `includes('healthy')` classified `exited:unhealthy` as running. Inside `stopAllApps` that only ever cost a no-op stop against an already-stopped application, which nobody saw. Shared with a confirmation prompt it does something worse — it names already-dead applications in a dialog whose entire job is to be accurate, and someone scanning that list for the one application that should not be in it is handed noise. `running:unhealthy` still counts, via the `running` branch: an application that is up and failing its health checks is still something an emergency stop should take down.

  **Observable behaviour change:** `stop_all_apps` no longer issues a stop against applications in a terminal-but-unhealthy state such as `exited:unhealthy`. Those calls were no-ops, so nothing that previously stopped will now stay up.
