...
│   └── docs-search.ts          # MiniSearch index over Coolify's published llms-full.txt
├── types/
...
docs/
├── coolify-openapi.yaml        # vendored upstream spec — ground truth for "does Coolify support X"
└── openapi-chunks/             # the same spec split by resource, generated by scripts/split-openapi-chunks.mjs
...
flowchart TB
    subgraph mcp["coolify-mcp process"]
        ...
        d["src/lib/docs-search.ts<br/>MiniSearch over Coolify's llms-full.txt"]
        a --> b
        b -.-> c
        a -.-> c
        a --> d
