...
- plants **canary secrets** (any value containing `CANARY`) in env vars,
  ...
  detectable; and
- records every request, exposing `mutations()` so a test can assert _nothing
  changed_, independent of how tools map to endpoints.
...
sees, and the read-only/destructive split used in assertions is _derived from
the server's own annotations table_, never hand-listed here.
...
_and_ Gemini clients (a Gemini-only schema bug already shipped once — server
issue #325).
...
- **Selection pass rate** is gated against a **per-provider baseline** (Claude
  ~0.9, Gemini Flash `0.45` — a coarse floor under its noisy 0.43–0.64 free-tier
  band; it under-chains `list→act`, see FINDINGS.md #2). The baseline is a
